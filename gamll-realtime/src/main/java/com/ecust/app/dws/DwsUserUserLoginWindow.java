...
/**
 * 1）读取 Kafka 页面主题数据 dwd_traffic_page_log
 * 2）转换数据结构
 * 3）过滤数据
 * 4）设置水位线
 * 5）按照 uid 分组
 ...
 * 8）写入 ClickHouse
 * <p>
 * 数据流 ： web/app -> nginx -> 日志服务器(.log) -> Flume -> Kafka(ODS) -> FlinkAPP(BaseLogAPP) -> Kafka(DWD) -> FlinkAPP -> Clickhouse
 * 程序：Mock(模拟产生日志数据，lg.sh) -> Flume(f1.sh) ->  Kafka(ZK)-> FlinkAPP(BaseLogAPP) -> Kafka(DWD)  -> DwdUserLoginWindow -> Clickhouse(DWS)
 */
