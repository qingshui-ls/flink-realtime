...
public class DimUtil {
    public static JSONObject getDimInfo(Connection conn, String tableName, String id) throws SQLException, InvocationTargetException, InstantiationException, IllegalAccessException {
     ...
     * TTL:        1day
     */
    public static JSONObject getDimInfo(Connection conn, String tableName, Tuple2<String, String>... columnNameAndValues) throws SQLException, InvocationTargetException, InstantiationException, IllegalAccessException {
        // 拼接从Redis中查询维度的key
        StringBuilder redisKey = new StringBuilder("dim:" + tableName.toLowerCase() + ":");
        // 拼接查询SQL
        StringBuilder selectSql = new StringBuilder("select * from " + GmallConfig.HBASE_SCHEMA + "." + tableName + " where ");
        ...
        if (dimJsonStr != null && dimJsonStr.length() > 0) {
            ...
            // 底层还是调用 PhoenixUtil，从phoenix表中进行查询
            List<JSONObject> dimList = PhoenixUtil.queryList(conn, selectSql.toString(), JSONObject.class);
...
// 根据维度查询条件  到维度表中查询维度数据
    public static JSONObject getDimInfoNoCache(Connection conn, String tableName, Tuple2<String, String>... columnNameAndValues) throws SQLException, InvocationTargetException, InstantiationException, IllegalAccessException {
        // 拼接查询SQL
        StringBuilder selectSql = new StringBuilder("select * from " + GmallConfig.HBASE_SCHEMA + "." + tableName + " where ");
        // 在java语句中，将可变长参数封装为数据  所以我们需要对数据进行遍历
        for (int i = 0; i < columnNameAndValues.length; i++) {
    ...
    public static JSONObject getDimInfo1(Connection conn, String tableName, String key) throws SQLException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        List<JSONObject> jsonObjects = PhoenixUtil.queryList(conn, sql, JSONObject.class);
        // 将查询到数据写入redis
